...
import Sha256Hash from 'js-sha256'
...
export default class OBSRemote extends EventEmitter {
	...
	async login(password) {
		console.log('tried to login with: ', password)
		const {authRequired, salt, challenge} = await this.send({'request-type': 'GetAuthRequired'}).then(d=>console.log('pass reply with: ',d))
